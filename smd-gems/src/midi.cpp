...
namespace 
{
	...
	*/
	...
	Value& get_or_add(std::unordered_map<Key, Value>& map, const Key& key, const Value& default_value) {
		auto it = map.find(key);
		if (it != map.end()) {
			return it->second;
		}
		return map.emplace(key, default_value).first->second;
	}
	...
	//int inst_offs(bool triple_pointers, int i)
	//{
	//	if (triple_pointers)
	...
	struct note
	{
		...
		note(int _c, int _x, int _t, bool _on, int _v, int _p, int _z, int _pitch) : x(_x + 12), t(_t), c(_c), v(_v - 30), p(_p),
			z(_z), pitch(_pitch), id(next_id++), on(_on) {
			update = false;
		}
...
#define lfprintf if(write_log) fprintf
	...
	{
		notes.emplace_back(c, x, at, true, volume, patch, priority, pitch);
	}

	void release(int c, int x, int at)
	{
		notes.emplace_back(c, x, at, false, volume, patch, priority, pitch);
	}

	void change_pitch()
	{
		notes.emplace_back(channel, 0, time, true, volume, patch, priority, pitch);
	...
	void parseseq(const ym::gems::IBank& in_bank, int offs)
	{
		auto&& reader = in_bank.create_reader(offs);
		for (;;)
		{
			if (time < 0 || (maxtime != 0 && time > maxtime))
				break;
			buff[0] = reader->read<std::uint8_t>();
			if (buff[0] >= 0xC0)
			{
				if (wasdelay)
				...
				lprintf("loop %X times\n", (int)buff[0]);
				{
					...
					if (times == 0x7F) // infinity
					{
						if (maxtime == 0) // if we don't know "full" period of whole track
						{
							...
							parseseq(in_bank, pos); // do one loop iteration to calculate loop period
	...
	int inst_type(const ym::gems::IBank& in_bank, int i)
	{
		return in_bank.get_instrument_type(i);
	}
	...
	void process_note(const ym::gems::IBank& in_bank, const note& n, int midi_channel)
	{
		...
		if (patches[n.c & 0xFF] != n.p && n.p >= 0)
		{
			track.push_back(0xC0 | midi_channel);
			track.push_back(n.p);
			track.push_back(0);

			patches[n.c & 0xFF] = n.p;
		}
		...
		getinst(n.p);
		int type = inst_type(in_bank, n.p);
	...
	class MidiConverter : public ym::gems::IConverter
	{
	public:
		void convert_sequence(const ym::gems::IBank& in_bank, std::uint16_t in_sequence, std::ostream& out_midi_stream, std::ostream& out_info_stream) const override
		{
			if (in_sequence < in_bank.sequences_num())
			{
				bool ptr3 = false;
				bool use_crt = false;
				bool pal_scale = false;

				in_bank.read_settings(ptr3, use_crt, pal_scale, play_songs);
				...
				for (maxtime = 0;;)
				{
					notes.clear();
					play_song.clear();
					play_song.push_back(in_sequence);
					play_time.clear();
					...
					for (int song = 0; song < play_song.size(); ++song)
					{
						...
						auto&& sequence_sequences = in_bank.get_sequence_sequences(play_song[song] * 2);

						for (int sequence_index = 0; sequence_index < sequence_sequences.size(); ++sequence_index)
						{
							patch = -1;
							pitch = 0;
							volume = 0; // loudest
							...
							wasdelay = false;
							wasduration = false;
							channel = sequence_index + (play_song[song] << 8);
							lprintf("parseseq %X %X\n", in_sequence, channel);
							parseseq(in_bank, sequence_sequences[sequence_index]);
						}
					}
					if (maxtime != 0)
						break;
				...
				out_midi_stream.write(reinterpret_cast<const char*>(buff), 14);
				...
				out_midi_stream.write(reinterpret_cast<const char*>(buff), 8);
				out_midi_stream.write(reinterpret_cast<const char*>(track.data()), track.size());
				...
				std::unordered_map<int, std::set<int>> channels_patches;
				...
				for (auto& note : notes)
				{
					if (pc != note.c)
					{
						...
						track.clear();
						// init new track;
						sprintf((char*)buff, "Patch %02X(%s)", note.p, inst_type_name[inst_type(in_bank, note.p)]);
					...
					process_note(in_bank, note, tracks_count & 0xF);

					if (note.p >= 0)
					{
						get_or_add(channels_patches, note.c, std::set<int>()).insert(note.p);
					}
				}
				if (!track.empty())
				{
				...
				out_midi_stream.seekp(11, std::ios::beg);

				buff[0] = tracks_count + 1;
				// fwrite(buff, 1, 1, out);
				out_midi_stream.write(reinterpret_cast<const char*>(buff), 1);
				...
				for (int z = 0; z < inst.size(); ++z)
				{
					...
					const unsigned char* ym = in_bank.get_instrument_raw(inst[z]);

					// fprintf(txt, "Patch %02X (%s):", inst[z], inst_type_name[inst_type(inst[z])]);
					out_info_stream << std::format("Patch {:02X} ({}):", inst[z], inst_type_name[inst_type(in_bank, inst[z])]);

					for (int zz = 0; zz < 5; ++zz)
					{
						// fprintf(txt, " %02X", (int)ym[zz]);
						out_info_stream << std::format(" {:02X}", (int)ym[zz]);
					}
				...
				for (int z = 0; z < dac.size(); ++z)
				{
					...
					out_info_stream << std::format("DAC ({}) {:02X} {:02X}\n", z, dac[z] >> 8, dac[z] & 0xFF);
				}
			}
		}
	};
...
namespace ym::gems
{
	...
	{
		return std::make_shared<MidiConverter>();
	}
}